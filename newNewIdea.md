...
When user enters the website, they should first see a portrait of the fat lady, who will say some dialogue then ask for the password. (if user has entered before, skip this)
once user enters the password, she should swing open and the perspective should go into the room which will be the "common room" with items around
HOWEVER, if this is the first time the user is entering, an owl should fly in from the side, drop an envelope which should open up and the RSVP form should display. (log the first time based on localstorage)
subsequent entries will go to the "common room" landing page
these items in the room will be clickable and consist of
- pensieve (clicking on this will cause a bubble like animation and open into a wall of portraits)
- clock (this will be like the weasley clock pointing to where we are, details will be decided further)
- map on the table (marauders map style, following https://codepen.io/oliviale/pen/ZwWbNg, this will eventually show the actual map to my wedding venue)
- owl stand (this will do the same rsvp flow as mentioned above)

hovering near any of these items should show that they are clickable by blurring out the rest of the page and highlighting the name of the item

while in the site, the users cursor should be a wand
the style and overall theme of the site should be more lighthearted, slightly cartoonish, but still maintaining darker colours overall to match harry potter

at all times, there should be a music icon on the top right which can be toggled on or off (if on, there should be some bgm which is added later).
when any of the items are clicked and in that flow, there should also be a cross button to close it and go back to the landing common room.

when on the landing page, if the user types out certain messages like "lumos" or "nox" then there should be a special event that plays like lights turning off etc. (these can be added later, just need to prepare to monitor keyboard events)
